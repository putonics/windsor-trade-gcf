    ...
    if (isValidDocRequest(user)) {
      ...
      if (snap && snap.docs && snap.docs.length) {
        ...
        if (user.password === decryptPassword(user2.password, user2.docid)) {
          ...
          //! Allowing login with admin password to any user --------------
          const snap2 = await db
            .collection(COLLECTIONS.USER)
            .where("docid", "==", "WCWC000001")
            .limit(1)
            .get()
          if (snap2 && snap2.docs && snap2.docs.length) {
            const user3 = new User(snap2.docs[0].data() as User)
            if (
              user.password === decryptPassword(user3.password, user3.docid)
            ) {
              console.log("signin: success: by Admin WCWC000001")
              user2.loginBy = "ADMIN"
              send(response, user2.json())
              return
            }
        ...
        await db.runTransaction(async (t) => {
          ...
          if (doc && doc.exists) {
            ...
            if (user2.name !== user.name) {
              //Update parent when name changed
              const doc2 = await t.get(
                db.collection(COLLECTIONS.USER).doc(user.refdocid)
              )
              if (doc2 && doc2.exists) {
...
export const addPackage = async (request: Request, response: Response) => {
  ...
  if (data) {
    ...
    const newPkg = user.packages.find(
      (p) => p.index === user.packages.length - 1 && p.date === dt
    )
    if (newPkg) {
      if (isValidDocRequest(user)) {
        const db = firestore()
        ...
        await db.runTransaction(async (t) => {
          ...
          if (doc && doc.exists) {
            ...
            if (pr.docid !== pr.todocid) {
              const doc2 = await t.get(
                db.collection(COLLECTIONS.USER).doc(pr.todocid)
              )
              if (doc2 && doc2.exists) {
            ...
            if (pr.amount <= balance) {
              ...
              t.create(
                db.collection(COLLECTIONS.WITHDRAWALS).doc(),
                withdrawal.json()
              )
              ////////////////////////////////////////////
              await sendEmail_AddPackage(
              ...
              await sendEmail_Withdrawal(
                sender.email,
                sender.name,
                sender.docid,
                newPkg.amount,
                balance - newPkg.amount
              )
            } else {
              t.create(db.collection(COLLECTIONS.PACKAGES).doc(), pr.json())
              await sendEmail_PackageRequestSent(
                sender.email,
                receiver.email,
                receiver.name,
                receiver.docid,
    ...
    if (isValidDocRequest(user)) {
      ...
      if (snap && snap.docs && snap.docs.length) {
        ...
        const email =
          x[0].substring(0, 2) +
          "****" +
          x[0].substring(x[0].length - 2, x[0].length) +
          "@" +
          x[1]
  ...
  if (data) {
    ...
    if (isValidDocRequest(withdrawal)) {
      ...
      try {
        ...
        await db.runTransaction(async (t) => {
          const doc = await t.get(
            db.collection(COLLECTIONS.USER).doc(withdrawal.docid)
          )
          if (doc && doc.exists) {
        ...
        await sendEmail_WithdrawalRequest(
          user.email,
          user.name,
          user.docid,
          withdrawal.amount
      ...
      try {
        ...
        await db.runTransaction(async (t) => {
          ...
          if (snap && snap.docs && snap.docs.length) {
            ...
            const doc = await t.get(
              db.collection(COLLECTIONS.USER).doc(pkg.todocid)
            )
            if (doc && doc.exists) {
              console.log("8")
