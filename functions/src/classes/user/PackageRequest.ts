...
export default class PackageRequest {
  ...
  constructor(pr?: PackageRequest) {
    this.docref =
      pr && pr.docref && pr.docref.id ? { id: pr.docref.id } : { id: "" }
  ...
  set(pr?: PackageRequest) {
    this.docref =
      pr && pr.docref && pr.docref.id ? { id: pr.docref.id } : this.docref
    this.appname = pr && pr.appname ? pr.appname : this.appname
    this.subscriberdocid =
      pr && pr.subscriberdocid ? pr.subscriberdocid : this.subscriberdocid
    ...
    this.tocountrycode =
      pr && pr.tocountrycode ? pr.tocountrycode : this.tocountrycode
  ...
  setReceiver(user: User) {
    ...
    this.tocountrycode = user.countrycode
      ? user.countrycode
      : this.tocountrycode
