...
export default class User {
  ...
  constructor(user?: User) {
    ...
    this.subscriberdocid =
      user && user.subscriberdocid ? user.subscriberdocid : ""
    ...
    this.groupA =
      user && user.groupA
        ? user.groupA.map((a) => new Subordinate(a))
        : new Array<Subordinate>()
    this.groupB =
      user && user.groupB
        ? user.groupB.map((b) => new Subordinate(b))
        : new Array<Subordinate>()
    this.packages =
      user && user.packages
        ? user.packages.map((p) => new Package(p))
        : new Array<Package>()
  ...
  updateProfile(user: User) {
    ...
    this.bankAccount =
      user && user.bankAccount ? new BankAccount(user.bankAccount) : null
    //////////////////////////////////////////////////////////////
    this.cryptoWalletAddress = user.cryptoWalletAddress
  ...
  json() {
    ...
    const monthlyTurnover =
      this.docid === ADMINDOCID ? this.monthlyTurnover : null
  ...
  isValid() {
    //first package should be greater than $15
    return (
      this.name.length > 4 &&
      this.email.includes("@") &&
      this.email.length > 4 &&
  ...
  addCompanyTurnover(pkg: Package) {
    //only admin
    if (this.docid === ADMINDOCID) {
      this.totalTurnover += pkg.amount
      const pkgDate = new Date(pkg.date)
      const mt = this.monthlyTurnover.find(
        (m) =>
          m.month === pkgDate.getMonth() && m.year === pkgDate.getFullYear()
      )
      if (mt) {
        mt.amount += pkg.amount
      } else {
        this.monthlyTurnover.push({
          month: pkgDate.getMonth(),
          year: pkgDate.getFullYear(),
          amount: pkg.amount,
        })
      }
    }
  ...
  private setTurnover(pkg: Package) {
    if (this.docid === ADMINDOCID) {
      ...
      const mt = this.monthlyTurnover.find(
        (m) => m.month === pkMonth && m.year === pkYear
      )
      if (mt) {
        mt.amount += pkg.amount
      } else {
  ...
  private async sendIncomeToParent(
    db: firestore.Firestore,
    t: firestore.Transaction,
    pkg: Package,
    level: number
  ) {
    //first level always starts with 1
    ...
    if (snap && snap.exists) {
      ...
      if (parent.docid === ADMINDOCID) {
        ...
        parent.income.absorbTopSubordinateByAdmin(
          this.docid,
          pkg,
          level,
          parent
  ...
  getWalletBalance(amount: number) {
    ...
    if (this && this.packages && this.packages.length) {
      ...
      this.packages.forEach((p) => {
        ...
        const diff = tym - pym
        sum += p.amount * ROI_PERCENTAGE * diff
      })
      sum +=
        this.income.totalReferralIncome +
