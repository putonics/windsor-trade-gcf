...
import { signup, checkEmail, checkReferer, signin, adminSignin, inactiveUsers, addPackage, load, sendOtp, requestWithdrawal, requestPackage, getWithdrawalList, serveWithdrawal, shareTurnover, getPendingWithdrawals, modify, getPendingPackages, servePackage, modifyAdmin } from './classes/user/index';
...
app.get('/test', (req, res) => { res.json({ msg: 'All okay!' }) })
app.get('/test-email/:email', async (req, res) => {
    const { email } = req.params
    const sent = await sendEmail(email, 'Testing Email', '',
        `
        <!DOCTYPE html>
        <html>
            <head>
